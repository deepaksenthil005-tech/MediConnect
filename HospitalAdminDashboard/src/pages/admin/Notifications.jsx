      ...
      <div className="flex flex-wrap items-center gap-2 md:gap-3 p-2 bg-slate-100/50 rounded-2xl w-full md:w-fit">
        {["ALL", "SYSTEM", "APPOINTMENTS"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 md:px-6 py-2 md:py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${filter === f
                ...
                <div className="grid grid-cols-3 gap-2 md:gap-3">
                  {["ALL", "PATIENTS", "DOCTORS"].map((t) => (
                    <button
                      ...
                      className={`py-2 md:py-3 text-[9px] md:text-[10px] font-black tracking-widest uppercase rounded-2xl border-2 transition-all ${formData.target === t
