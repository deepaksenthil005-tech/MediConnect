      ...
      >
        <div className="p-6 flex items-center justify-between">
          <Link to="/admin" className="flex items-center space-x-2">
            ...
            {isSidebarOpen && <span className="text-xl font-bold text-slate-900 tracking-tight">MediConnect</span>}
          </Link>
        </div>
        ...
        >
          {isSidebarOpen ? <ChevronLeft className="h-4 w-4 text-slate-400" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}
        </button>
      </motion.aside>
            ...
            <div className="relative">
              ...
              >
                <div className="w-9 h-9 rounded-md bg-white/10 overflow-hidden flex-shrink-0 flex items-center justify-center border border-white/10 group-hover:border-white/30 transition-all">
                  {user?.image_url ? (
                    <img src={user.image_url} alt="Admin" className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-white font-black text-sm">{user?.name?.[0] || 'A'}</span>
                  )}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-[10px] font-black text-white leading-none uppercase tracking-widest">{user?.name?.split(' ')[0] || 'Admin'}</p>
                  <p className="text-[8px] font-bold text-white/50 uppercase tracking-[0.2em] leading-none mt-1">Management</p>
                </div>
              </button>
              ...
              <AnimatePresence>
                {profileMenuOpen && (
                  ...
                  >
                    ...
                    {[
                      { icon: User, label: 'Identity Settings', path: '/admin/profile' },
                      { icon: Bell, label: 'Alert Protocol', path: '/admin/notifications' },
                      { icon: Settings, label: 'System Control', path: '/admin/settings' },
                    ].map((item) => (
                      <button 
                        key={item.label}
        ...
        <footer className="bg-white border-t border-slate-200 py-4 px-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-slate-500 font-medium">
              © {new Date().getFullYear()} MediConnect Admin Portal. All rights reserved.
            </p>
