...
const menuItems = [
  ...
  { to: '/dashboard/book-appointment', icon: CalendarPlus, label: 'Book Appointment' },
  { to: '/dashboard/my-appointments', icon: CalendarCheck, label: 'My Appointments' },
...
export default function PatientLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      <aside className={`premium-sidebar hidden lg:flex flex-col sticky top-0 h-screen z-30 transition-all duration-300 ${sidebarCollapsed ? 'w-20' : 'w-[280px]'}`}>
        <Link to="/dashboard" className="patient-dashboard__sidebar-logo">
          <div className="patient-dashboard__sidebar-logo-icon">
            <CalendarCheck size={18} />
          </div>
          {!sidebarCollapsed && <span className="patient-dashboard__sidebar-logo-text">MediConnect</span>}
        </Link>
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <button type="button" className="nav-link-premium w-full text-rose-500 hover:bg-rose-50 hover:text-rose-600 mt-4 group" onClick={handleLogout}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="premium-header h-20 px-8">
          <div className="flex items-center gap-4">
            <button
             ...
             <Link to="/dashboard" className="lg:hidden flex items-center space-x-2">
               ...
               <span className="text-xl font-bold text-slate-900 tracking-tight font-black">MediConnect</span>
            </Link>
          </div>
                    ...
                    <div className="max-h-[400px] overflow-y-auto p-2 no-scrollbar">
                      {loadingNotifications ? (
                        ...
                        <div className="p-8 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">No notifications</div>
                         ...
                         notifications.map((n) => (
                           <div key={n.id} className="p-4 rounded-md hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                             <div className="text-sm font-black text-slate-900 mb-1">{n.title}</div>
                             <div className="text-[11px] font-medium text-slate-500 line-clamp-2 leading-relaxed">{n.message}</div>
                          </div>
                        ))
                      )}
                    </div>
               ...
               >
                <div className="w-9 h-9 rounded-md bg-emerald-100 border border-emerald-200 overflow-hidden flex-shrink-0 flex items-center justify-center">
                  {user?.image_url ? (
                    ...
                    <span className="text-emerald-600 font-black text-sm">{user?.name?.[0] || 'P'}</span>
                  )}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-[10px] font-black text-white leading-none uppercase tracking-widest">{user?.name?.split(' ')[0] || 'Patient'}</p>
                  <p className="text-[8px] font-bold text-white/50 uppercase tracking-[0.2em] leading-none mt-1">Health Dossier</p>
                </div>
              </button>
                  ...
                  <motion.div 
                   ...
                   >
                    ...
                    {[
                      ...
                      { icon: Heart, label: 'Clinical Vitals', path: '/dashboard/health-report' },
