...
export default function AppointmentsManagement() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const [aptData, docData] = await Promise.all([localDb.getAppointments(), localDb.getDoctors()]);
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    await localDb.updateAppointmentStatus(id, newStatus);
    fetchData();
  };

  const handleViewPatientReport = async (patientId) => {
    const reportData = await localDb.getHealthReport(patientId);
                           ...
                           if(file) {
                             ...
                             reader.onloadend = async () => {
                               try {
                                 ...
                                 await localDb.addMedicalRecord(selectedPatientReport.patient_id, record);
