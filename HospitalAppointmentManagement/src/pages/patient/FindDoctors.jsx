...
const SPECIALIZATIONS = ['All', 'General Physician', 'Cardiologist', 'Neurologist', 'Pediatrician', 'Dermatologist', 'Psychiatrist'];
      ...
      <AnimatePresence>
        {selectedDoctor && (
          <DoctorModal 
            ...
            onBook={() => navigate(`/dashboard/book-appointment?doctor=${selectedDoctor.id}`)}
          />
        )}
      </AnimatePresence>
    </div>
 ...
 */
function SearchFilterHeader({ searchQuery, setSearchQuery, filters, setFilters, resultsCount }) {
  return (
    <div className="premium-card !p-6">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
        <div>
          <h3 className="text-2xl font-black text-slate-900 tracking-tight">Find Your Specialist</h3>
          <p className="text-slate-400 font-medium text-sm mt-1">Discover top-rated doctors and book your visit in seconds.</p>
        </div>
      ...
      <div className="relative p-8 pb-0">
        ...
        <div className="flex gap-6 mb-8">
          ...
          <div className="relative shrink-0">
            <div className="w-24 h-24 rounded-md overflow-hidden bg-slate-100 border-4 border-white shadow-md">
              <img 
                src={doc.image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(doc.name)}&size=100`} 
                alt={doc.name} 
          ...
          <div className="min-w-0 pt-2">
            <h4 className="text-lg font-black text-slate-900 truncate leading-tight mb-1">{doc.name}</h4>
            <p className="text-emerald-600 font-bold text-xs uppercase tracking-wider">{doc.specialization}</p>
            <div className="flex items-center gap-1.5 mt-3">
              <div className="flex">
                {[1,2,3,4,5].map(s => (
                  <Star key={s} size={10} fill={s <= 4 ? "#f59e0b" : "none"} color="#f59e0b" />
                ))}
              </div>
              <span className="text-xs font-black text-slate-700">4.8</span>
              <span className="text-[10px] font-bold text-slate-400">(120+ Reviews)</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 p-4 rounded-md bg-slate-50 border border-slate-100 mb-8">
          <div>
            ...
            <p className="text-sm font-black text-slate-900">{doc.experience || 8}+ Years</p>
          </div>
          <div className="border-l border-slate-200 pl-4">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Fee</p>
            <p className="text-sm font-black text-emerald-600">₹{doc.fee || 500}</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 md:p-10 h-[600px] overflow-y-auto">
          ...
          <div className="flex flex-col md:flex-row gap-8 mb-10">
            ...
            <div className="pt-4">
              ...
              <p className="text-emerald-600 font-bold uppercase tracking-widest text-sm mb-4">{doctor.specialization}</p>
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-md">
                  <Award size={16} className="text-amber-500" />
                  <span className="text-xs font-black text-slate-700">{doctor.experience || 8}+ Yrs Exp</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-md">
                  <Star size={16} className="text-amber-500" />
                  <span className="text-xs font-black text-slate-700">4.8 (120+ Reviews)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              {/* About Text */}
              <div>
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">About Doctor</h4>
                <p className="text-sm font-medium text-slate-600 leading-relaxed">
                  Dr. {doctor.name.split(' ').pop()} is a highly skilled {doctor.specialization.toLowerCase()} with extensive clinical experience. Dedicated to providing compassionate, evidence-based care tailored to each patient's unique needs.
                </p>
              </div>
              
              {/* Timing */}
              <div>
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Consultation Hours</h4>
            ...
            <div className="bg-slate-50 rounded-md p-6 border border-slate-100">
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Patient Feedback</h4>
              <div className="space-y-6">
                {[
                  { name: 'Rahul S.', rating: 5, text: 'Great experience, very professional.' },
                  { name: 'Ananya K.', rating: 4, text: 'Good listener and provided clear advice.' }
                ].map((rev, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-slate-900 uppercase tracking-tighter">{rev.name}</span>
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map(s => <Star key={s} size={8} fill={s <= rev.rating ? "#f59e0b" : "none"} color="#f59e0b" />)}
                      </div>
                    </div>
...
function LoadingSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {[1,2,3,4,5,6].map(i => (
        <div key={i} className="h-[450px] bg-slate-100 rounded-md animate-pulse" />
      ))}
    </div>
  );
}
